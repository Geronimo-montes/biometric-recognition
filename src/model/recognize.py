...
from utils.settings import load_names_list
...
def detect_face(imagen) -> List[Tuple]:
    ...
    from utils.settings import MIN_SIZE
...
def recognize_video():
    for name in os.listdir(PATH_DATA_MODEL):
        file = os.path.join(PATH_DATA_MODEL, name)
